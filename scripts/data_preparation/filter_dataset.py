...
def parse_args():
    ...
    parser.add_argument(
        "--img_dir", type=str, required=True, help="Source image directory"
    )
    parser.add_argument(
        "--label_dir", type=str, required=True, help="Source label directory"
    )
    parser.add_argument(
        "--output_img_dir", type=str, required=True, help="Output image directory"
    )
    parser.add_argument(
        "--output_label_dir", type=str, required=True, help="Output label directory"
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def identify_rare_classes(
    class_counts: Dict[int, int], threshold_percent: float
    ...
    for class_id in sorted(class_counts.keys()):
        ...
        if percentage < threshold_percent:
            ...
            print(
                f"❌ Class {class_id}: {count} instances ({percentage:.2f}%) - FILTERED"
            )
        else:
...
def save_filter_report(
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Retention Rate**: {stats['copied_images']/stats['total_images']*100:.1f}%\n\n"
        )
        ...
        f.write(
            f"- **Box Retention Rate**: {stats['total_boxes_filtered']/stats['total_boxes_original']*100:.1f}%\n\n"
        )
...
def main():
    ...
    if args.analysis_json:
        ...
        with open(args.analysis_json, "r") as f:
            ...
            class_counts = {
                int(k): v
                for k, v in analysis["class_distribution"]["class_counts"].items()
            }
    else:
    ...
    rare_classes, valid_classes = identify_rare_classes(
        class_counts=class_counts, threshold_percent=args.threshold
    )
    ...
    print(f"\n🔄 Filtering dataset...")
    print(
        f"   Mode: {'Remove rare boxes, keep images with valid boxes' if args.remove_rare_from_labels else 'Skip images with any rare boxes'}"
    )
...
# python filter_dataset_by_class.py --img_dir dataset/dataset_v0/images --label_dir dataset/dataset_v0/labels_new --output_img_dir dataset/dataset_filtered/images --output_label_dir dataset/dataset_filtered/labels --threshold 1.0 --analysis_json analysis_results_new/dataset_statistics.json

# Keep images, just remove rare class boxes:
# python filter_dataset_by_class.py --img_dir dataset/dataset_v0/images --label_dir dataset/dataset_v0/labels_new --output_img_dir dataset/dataset_filtered/images --output_label_dir dataset/dataset_filtered/labels --threshold 1.0 --remove_rare_from_labels --analysis_json analysis_results_new/dataset_statistics.json
