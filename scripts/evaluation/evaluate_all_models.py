...
def find_dataset_config_from_args(model_dir: Path, processed_dir: Path):
    ...
    if not full_data_config_path.exists():
        if (project_root / "configs" / Path(data_config_path).name).exists():
            full_data_config_path = (
                project_root / "configs" / Path(data_config_path).name
            )
        else:
            return None
...
def main():
    ...
    parser.add_argument(
        "--runs-dir", type=str, default="runs", help="Directory containing runs"
    )
    parser.add_argument(
    ...
    for weight_path in sorted(best_weights):
        ...
        names_str = (
            ",".join(str(n) for n in names) if isinstance(names, list) else str(names)
        )
        ...
        cmd = [
            sys.executable,
            "scripts/evaluate_yolo_standalone.py",
            "--model",
            str(weight_path),
            "--dataset-dir",
            str(dataset_dir),
    ...
    if results_list:
        ...
        with open(summary_file, "w") as f:
            f.write(
                f"{'Model':<40} | {'mAP50-95':<10} | {'mAP50':<10} | {'Precision':<10} | {'Recall':<10}\n"
            )
            f.write("-" * 90 + "\n")
            for r in results_list:
                f.write(
                    f"{r['Model']:<40} | {r['mAP50-95']:.4f}     | {r['mAP50']:.4f}     | {r['Precision']:.4f}     | {r['Recall']:.4f}\n"
                )
