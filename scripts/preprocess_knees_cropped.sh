#!/bin/bash
# Preprocess knees_cropped dataset with all presets
...
# Paths
input_base = project_root / 'datasets/dataset_knees_cropped'
output_base = project_root / 'datasets/data_processed_knees'

print('='*60)
print('PREPROCESSING KNEES CROPPED DATASET')
print('='*60)
...
for preset_name, (desc, pipeline) in presets.items():
    ...
    for label_dir_name in ['labels', 'labels_new', 'labels_4class', 'labels_8class', 'labels-knee']:
