...
class KneePipeline:
    ...
    def process(self, image_rgb: np.ndarray, knee_conf: float = 0.25, grade_conf: float = 0.25):
        ...
        for i, box in enumerate(knee_result.boxes):
            ...
            # 2. Grade KL on Crop
            # IMPORTANT: The grading model expects the specific crop.
            ...
            if grade_results:
                ...
                if len(g_res.boxes) > 0:
                    # Detection/Classification model usually returns box for classification if using YOLO classify or detect
                    # If it's a YOLO-Detect model trained on single object per image (the knee), it might return a box covering the crop?
                    # Or if it's YOLO-Classify, the output structure is different (probs).
                    # Assuming the user trained a DETECT model for grading (as per previous context of 5-class detection).
                    # We pick the highest confidence box in the crop.
