"""
KIOCMIL Dataset - Current Active Version (V3)

This is the main dataset implementation, based on V3.
For older versions, see src/datasets/versions/

Enhanced for CADA Architecture with bbox information for deformable attention:
...
Version History:
- V3 (current): Added bbox support for deformable attention
- V2: Enhanced preprocessing and augmentation
...
from src.datasets.kiocmil_transforms import normalize_patch, patch_to_tensor


class KiocmilDataset(Dataset):
    """
    KIOCMIL Dataset with bbox information for deformable attention.
...
def collate_kiocmil(batch: List[Dict]) -> Dict:
    """
    Custom collate function for KIOCMIL dataset.
