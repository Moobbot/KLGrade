...
def stratified_split(
    ...
    for stem in tqdm(remaining, desc="Assigning images"):
        ...
        test_need = ((n_total - n_train - n_val) - current_test) / (n_total - n_train - n_val)
        ...
        "summary": {
            ...
            "splits": {
                "train": {
                    ...
                    "percentage": round(len(train_stems) / (len(train_stems) + len(val_stems) + len(test_stems)) * 100, 2)
                },
                "val": {
                    "num_images": len(val_stems),
                    "percentage": round(len(val_stems) / (len(train_stems) + len(val_stems) + len(test_stems)) * 100, 2)
                },
                "test": {
                    "num_images": len(test_stems),
                    "percentage": round(len(test_stems) / (len(train_stems) + len(val_stems) + len(test_stems)) * 100, 2)
                }
            }
        },
        "class_distribution": {
    ...
    for cls in sorted(total_dist.keys()):
        ...
        split_info["class_balance"][f"class_{cls}"] = {
            ...
            "train": {
                ...
                "percentage": round(train_dist.get(cls, 0) / total_count * 100, 2) if total_count > 0 else 0
            },
            "val": {
                "count": val_dist.get(cls, 0),
                "percentage": round(val_dist.get(cls, 0) / total_count * 100, 2) if total_count > 0 else 0
            },
            "test": {
                "count": test_dist.get(cls, 0),
                "percentage": round(test_dist.get(cls, 0) / total_count * 100, 2) if total_count > 0 else 0
            }
        }
...
# python split_dataset.py --img_dir dataset\dataset_v1\images --label_dir dataset\dataset_v1\labels --out_dir splits --train 0.7 --val 0.15 --test 0.15 --seed 42
# python split_dataset.py --img_dir dataset\dataset_v1\images --label_dir dataset\dataset_v1\labels_new --out_dir splits --train 0.7 --val 0.15 --test 0.15 --seed 42